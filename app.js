...
function filteredQuestions(){
  ...
  return healthQuestions.filter(q=>{
    const categoryOk=activeCategory==='Toutes'||q.category===activeCategory;
    if(!categoryOk) return false;
    if(!term) return true;
    const haystack=normalizeText(`${q.title||''} ${q.keywords||''} ${q.category||''} ${q.answer||''}`);
    const words=term.split(/\s+/).filter(w=>w.length>2);
    return haystack.includes(term) || words.some(w=>haystack.includes(w));
  });
}
...
if(filters)filters.addEventListener('click',e=>{
  ...
  if(!b)return;
  activeCategory=b.dataset.category||'Toutes';
...
if(menuButton&&mobileNav){
  ...
  menuButton.addEventListener('click',()=>{const isOpen=mobileNav.classList.toggle('open');menuButton.setAttribute('aria-expanded',String(isOpen));menuButton.textContent=isOpen?'×':'☰';});
  mobileNav.addEventListener('click',e=>{if(e.target.closest('a')){mobileNav.classList.remove('open');menuButton.setAttribute('aria-expanded','false');menuButton.textContent='☰';}});
}
