...
const notFoundBody=(renderer.match(/function failNotFound\(\)\{([^}]*)\}/)||[])[1]||'';
const temporaryBody=(renderer.match(/function failTemporary\(\)\{([^}]*)\}/)||[])[1]||'';
const notFoundSetsNoindex=/noindex,follow/.test(notFoundBody);
const temporarySetsNoindex=/noindex,follow/.test(temporaryBody);
const examplesOk=rows.every(r=>r.inCanonical&&r.inSitemap);
const ok=canonical.length>0&&missingSitemap.length===0&&examplesOk&&notFoundSetsNoindex&&!temporarySetsNoindex;
const result={ok,canonicalCount:canonical.length,examples:rows,sitemapMissingCount:missingSitemap.length,sitemapMissingIds:missingSitemap,notFoundSetsNoindex,temporarySetsNoindex};
