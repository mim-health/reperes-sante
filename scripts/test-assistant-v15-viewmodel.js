...
const checks=[
 {q:'cystite',status:'match',primary:'cystite-femme',complements:[]},
...
const assistant=read('assistant-alpha.html'),widget=read('maca-assistant-widget.js'),library=read('fiches.html'),entry=read('corpus-v2-browser-entry.js'),home=read('index.html'),loader=read('corpus-loader.js');
const canonicalCount=c.MACA_CANONICAL_CORPUS.length;const hasCystitis=c.MACA_CANONICAL_CORPUS.some(q=>q&&q.id==='cystite-femme');
const productionWiring={
 ...
 assistantLoadsSearch:assistant.includes("load('search-v15-lab-candidate.js?v=20260908-v15-2')"),
 assistantLoadsViewModel:assistant.includes("load('assistant-v15-viewmodel-candidate.js?v=20260908-v15-2')"),
 assistantUsesViewModel:assistant.includes('MACA_ASSISTANT_V15_VIEWMODEL.build(query)'),
 widgetVersion:widget.includes('assistant-alpha.html?v=20260908-v15-2'),
 libraryVersion:library.includes('corpus-loader.js?v=20260908-bundle5')&&library.includes('maca-assistant-widget.js?v=20260908-v15-2')&&library.includes('assistant-alpha.html?v=20260908-v15-2')&&library.includes('corpus-v2-browser-entry.js?v=20260908-v15-2'),
 homeWidgetVersion:entry.includes('maca-assistant-widget.js?v=20260908-v15-2')&&home.includes('corpus-v2-browser-entry.js?v=20260908-v15-1')
};
const officialGate=c.MACA_SEARCH_V2.__macaV15LanguageFix===true;const wiringOk=Object.values(productionWiring).every(Boolean);const ok=rows.every(r=>r.pass)&&officialGate&&wiringOk&&canonicalCount===255&&hasCystitis;console.log(JSON.stringify({ok,rows,officialGate,canonicalCount,hasCystitis,productionWiring,viewModelVersion:c.MACA_ASSISTANT_V15_VIEWMODEL.version},null,2));if(!ok)process.exit(1);
