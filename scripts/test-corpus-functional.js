...
const keywordsText=v=>Array.isArray(v)?v.join(' '):String(v||'');
...
function find(term){const t=norm(term);return cards.filter(c=>norm([c.id,c.title,c.question,keywordsText(c.keywords),c.answer].join(' ')).includes(t));}
