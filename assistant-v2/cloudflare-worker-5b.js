...
export default {
  async fetch(request,env){
    ...
    try{
      ...
      if(!ranked.length||ranked[0].similarity<MIN_GATE)return response(origin,allowed,200,{status:'abstain',answer:'',category:null,cards_used:[],selected_cards:selectedCards,scope_note:'',pilot_meta:{grounding:'not_applicable'}});
      const raw=await synthesize(env,question,selected);
      const checked=validate(raw,selected);
      if(!checked.ok)return response(origin,allowed,200,{status:'abstain',answer:'',category:null,cards_used:[],selected_cards:selectedCards,scope_note:'',pilot_meta:{grounding:'contract_rejected'}});
      const result=checked.result;
      if(!(await grounding(env,result,cardById)))return response(origin,allowed,200,{status:'abstain',answer:'',category:null,cards_used:[],selected_cards:selectedCards,scope_note:'',pilot_meta:{grounding:'rejected'}});
      const cardsUsed=result.cards_used.map(id=>{const c=cardById.get(id);return {id,title:c?.title||id,url:c?.url||''};});
      return response(origin,allowed,200,{status:result.status,answer:result.answer,category:result.category,cards_used:cardsUsed,selected_cards:selectedCards,scope_note:result.scope_note,pilot_meta:{grounding:result.status==='answer'?'supported':'not_applicable',corpus_cards:corpus.cardCount}});
    }catch(e){return response(origin,allowed,503,{error:'pilot_temporarily_unavailable'});}
  }
};
