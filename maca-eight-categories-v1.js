...
(function(){
  ...
  function init(){
    ...
    let active='Toutes';box.innerHTML=markup(active);canonicalizeCards();
    box.addEventListener('click',e=>{const b=e.target.closest('.filter-chip');if(!b)return;const wanted=b.dataset.category;if(!['Toutes',...PUBLIC].includes(wanted))return;e.preventDefault();e.stopImmediatePropagation();active=wanted;box.querySelectorAll('.filter-chip').forEach(x=>x.classList.toggle('active',x.dataset.category===active));canonicalizeCards();grid.querySelectorAll('.qa-card').forEach(card=>card.style.display=(active==='Toutes'||categoryOf(card)===active)?'':'none');if(input){input.dataset.scope=active;input.placeholder=active==='Toutes'?'Rechercher une question santé…':`Rechercher dans « ${active} »…`;}document.querySelector('#questions')?.scrollIntoView({behavior:'smooth',block:'start'});},true);
